...
pub mod vesting_program {
    ...
    pub fn create_vesting_account(ctx: Context<CreateVestingAccount>) -> Result<()> {
        ...
        *ctx.accounts.vesting_account = VestingAccount {
        ...
        msg!(
            "Vesting account created for company: {}",
            vesting_account.company_name
        );
        msg!("Owner: {}", vesting_account.owner);
        msg!("Mint: {}", vesting_account.mint);
        msg!(
            "Treasury ATA (PDA): {}",
            vesting_account.treasury_token_account
        );
...
pub struct CreateVestingAccount<'info> {
    // 1. The Signer (pays for accounts and authorizes creation)
    #[account(mut)] // Needs to be mutable to sign and deduct lamports for rent
    pub signer: Signer<'info>,

    #[account(
        init, // Marks this account for creation
        payer = signer, // The 'signer' account pays the rent
        space = ANCHOR_DISCRIMINATOR
    )]
    pub vesting_account: Account<'info, VestingAccount>, // The account itself, typed using our struct
}
